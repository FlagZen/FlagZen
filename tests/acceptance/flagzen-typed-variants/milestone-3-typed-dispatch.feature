Feature: Typed proxy dispatch for feature flags
  ...
  Scenario: Boolean proxy dispatches true to matching variant
    ...
    And variants for boolean true and false
  ...
  Scenario: Typed dispatch with context accessor
    ...
    When the developer resolves "RetryStrategy" without explicit context
  ...
  Scenario: Long proxy dispatches to matching variant
    ...
    When the developer resolves "RateLimiter" through the dispatcher
  ...
  Scenario: Double proxy dispatches via approximate matching with default tolerance
    ...
    And a flag provider returning double 0.0999999999 for "sampling-ratio"
  ...
  Scenario: Double proxy selects first matching variant when ranges overlap
    Given a feature "SamplingStrategy" with type DOUBLE
    And a variant "LowSampling" with double value 0.1 and tolerance 0.05
    And a variant "NearLowSampling" with double value 0.12 and tolerance 0.05
